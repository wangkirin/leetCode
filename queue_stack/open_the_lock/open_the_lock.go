...
func openLock(deadends []string, target string) int {
	queue := []string{}
	deadendmap := make(map[string]bool)
	...
	for len(queue) > 0 {
		...
		for j := 0; j < 4; j++ {
			origin := b[j] // 备份下原始的字符
			// 正向转动转盘
			b[j] = (b[j]-'0'+1)%10 + '0' // '0'~'9'的字符减去'0' 变为整型，来和1作加减，外边再 + '0'又转为字符
			nextstep = append(nextstep, string(b))
			b[j] = origin // 恢复原始状态
			// 反向转动转盘
			b[j] = (b[j]-'0'+9)%10 + '0' // 如果是-1会出现负数情况，不好处理。循环左移的技巧
			nextstep = append(nextstep, string(b))
			b[j] = origin
		}
		for _, s := range nextstep {
			if _, ok := visited[s]; !ok && !deadendmap[s] {
